...
TEST(DFSTests, DISABLED_DFSIterative_GivenEmptyGraph_ReturnsEmptyPath)
{
    std::unordered_map<int, std::set<int>> graph;
    const std::vector<int> &path = DFSIterative(graph, 0, 1);
    EXPECT_EQ(0, path.size());
}

TEST(DFSTests, DISABLED_DFSIterative_GivenValidGraph_ReturnsCorrectPath)
{
    std::unordered_map<int, std::set<int>> graph;
    graph.emplace(std::pair<int, std::set<int>>(1, {2, 3, 6}));
    graph.emplace(std::pair<int, std::set<int>>(2, {3}));
    ...
    graph.emplace(std::pair<int, std::set<int>>(5, {}));
    graph.emplace(std::pair<int, std::set<int>>(6, {}));
    const std::vector<int> &path = DFSIterative(graph, 1, 5);
    ASSERT_EQ(5, path.size());
    EXPECT_EQ(path[0], 1);
    EXPECT_EQ(path[1], 2);
    EXPECT_EQ(path[2], 3);
